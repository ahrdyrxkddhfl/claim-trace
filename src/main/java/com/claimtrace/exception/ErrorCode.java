 ...
 * 어느 설계 불변조건을 강제하는지가 API 응답에 그대로 드러나야 하고, 그 대응이
...
public enum ErrorCode {
    ...
    /** E-4 · INV-4 · 개입 규칙에 걸린 청구를 승인 없이 확정하려 한 경우. */
    DUAL_CHECK_REQUIRED(
     ...
     *
     * <p>INV-4 는 규칙 조건에 해당하는 건이 <b>단독으로</b> 확정될 수 없다고
    ...
    /**
     * INV-4 · 규칙이 지정한 승인 권한이 없는 사용자가 승인하려 한 경우.
     *
     * <p>승인 가능 역할은 규칙의 {@code approverRoles} 에 데이터로 들어 있다.
     ...
     * 그래서 특정 불변조건 번호를 달지 않는다. 번호를 달면 "설계 불변조건이
    ...
    /**
     * 개입 규칙의 발동 조건 정의가 잘못된 경우.
     *
     * <p>요청자의 잘못이 아니라 운영 데이터의 오류이므로 500 이다. 조건을
     * 해석할 수 없는 규칙을 조용히 건너뛰지 않고 요청 전체를 실패시키는
     * 이유는, 그 규칙이 규제가 요구하는 통제 수단이기 때문이다. 통제가
     * 적용되지 않은 채 확정이 성공하는 것이 오류 응답보다 훨씬 위험하다.
     */
    RULE_DEFINITION_INVALID(
            "개입 규칙의 발동 조건을 해석할 수 없습니다", null, HttpStatus.INTERNAL_SERVER_ERROR);
